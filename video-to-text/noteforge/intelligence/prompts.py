...
def build_extraction_prompt(episode_name: str, content: str) -> str:
    ...
    return (
        ...
        f"```\n\n"
        f"## 输出要求\n\n"
        f"请直接输出提取结果正文，不要输出任何思考过程、规划语言（如「首先」「然后」「"
        f"哦对」「等下」「这个应该叫」）或对任务指令的复述。\n\n"
        f"## 笔记内容\n\n{content}"
    )
...
def build_merge_prompt(extractions: str, contradictions: str = "",
    ...
    return (
        ...
        "2. **发现关联** — 找出不同集之间的知识关联\n"
        "3. **标注来源** — 每个知识点标注来源：引用提取结果中「### 」行后的源笔记标题"
        "（如《互联网泡沫、量化崛起、沃什首秀，投资30年的美国往事》《对话Calvin：拆解量化交易炼金术》）。"
        "标题过长时用其辨识性前段（如《互联网泡沫、量化崛起…》）；仅当源笔记标题本身以「第X集/epXX」命名时用集数。\n"
        "4. **保留原话** — 金句原样保留\n"
